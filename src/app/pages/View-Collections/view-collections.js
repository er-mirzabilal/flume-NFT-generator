...
const ViewCollection = () => {
  ...
  const handleDeleteTrue = async () => {
    ...
    if (popup.show && popup.id) {
      await deleteCollection(popup.id)
        .then((response) => {
          getColections()
            .then((response) => {
    ...
    return (
      <div class="w-80 rounded-xl m-2 p-4 shadow-lg cursor-pointer hover:shadow-2xl relative">
        ...
        <div class="flex flex-row pt-2 justify-between">
          <h3 class="text-third">{collection.edition}</h3>
          <p class="bg-secondary rounded-md text-white px-2 text-sm self-center">
            {collection.status}
          </p>
        </div>
      </div>
      ...
      <section class="bg-gray-100">
        <div class="max-w-screen-2xl w-11/12 mx-auto flex sm:flex-nowrap flex-wrap">
          <div class="mt-6 lg:mt-0 pr-0 sm:pr-6 text-left self-center">
            <h1 class="text-primary xl:text-6xl lg:text-5xl md:text-4xl text-3xl mb-3">
              Create or Monitor
            </h1>
          ...
          <div class="my-5 mr-0 ml-auto shrink-0 text-right">
            <img
              class="-mb-20 w-96 h-auto md:w-full md:h-auto rounded"
              src={viewCollection1img}
            />
          </div>
        </div>
      </section>
      <section>
        ...
        >
         ...
         Delete
            ...
            <Button
              startIcon={
                deleting ? <CircularProgress size="1.4rem" /> : <DeleteIcon />
              }
              variant="contained"
              disabled={deleting}
              color="primary"
